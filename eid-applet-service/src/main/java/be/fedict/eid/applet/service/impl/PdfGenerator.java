	...
	public byte[] generatePdf(EIdData eIdData) throws DocumentException {
		...
		if (null != eIdData && null != eIdData.getIdentity()) {
			if (null != eIdData.getPhoto()) {
				...
				// TODO: to be tested
				/*
				 * try { Image barcodeImage =
				 * createBarcodeImage(identity.nationalNumber,
				 * identity.cardNumber);
				 * 
				 * barcodeImage.setAlignment(Element.ALIGN_CENTER); Paragraph
				 * barcodePara = new Paragraph(); barcodePara.add(barcodeImage);
				 * 
				 * document.add(barcodeImage); } catch (Exception e) {
