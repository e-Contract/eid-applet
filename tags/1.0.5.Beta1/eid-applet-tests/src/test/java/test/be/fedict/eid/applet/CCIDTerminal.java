...
public class CCIDTerminal {
	...
	private void setEndPoints() {
		usb_endpoint_descriptor [] usbEndPointDescriptors = (usb_endpoint_descriptor[])usbInterfaceDescriptor.endpoint.toArray(usbInterfaceDescriptor.bNumEndpoints);
		for (int i = 0; i < usbEndPointDescriptors.length;i++) {
			...
			if ((usbEndPointDescriptors[i].bmAttributes & 3) == 2) 				 // Bulk
				if ((usbEndPointDescriptors[i].bEndpointAddress & 0x80) == 0x80) // Bulk IN
	...
	public void open() {
		...
		if (usbDevHandle == null) {
			...
			throw new RuntimeException(String.format("usb_open device failed (%d/%d)", 
					busNumber,deviceNumber));
		}
		...
		if (rv != 0) {
			...
			throw new RuntimeException(String.format("set configuration failed (%d/%d): %d", 
					busNumber,deviceNumber,rv));
		}
/*		rv = LibUSB.libUSB.usb_set_altinterface(usbDevHandle, usbInterfaceDescriptor.bAlternateSetting);
		if (rv != 0) {
			LOG.debug(String.format("set alternative interface failed (%d/%d): %d", 
					busNumber,deviceNumber,rv));
			throw new RuntimeException(String.format("set alternative interface failed (%d/%d): %d", 
					busNumber,deviceNumber,rv));
		}*/
		...
		if (rv != 0) {
			LOG.debug(String.format("claim interface failed (%d/%d): %d", 
					busNumber,deviceNumber,rv));
			throw new RuntimeException(String.format("claim interface failed (%d/%d): %d", 
					busNumber,deviceNumber,rv));
		}
	}
	public void close () {
	...
	public byte[] read() {
		...
		int rv = LibUSB.libUSB.usb_bulk_read(usbDevHandle, endPointBulkIn, buffer, buffer.length, 5000); // timeout: 5 secs
		if (rv < 0) {
			LOG.debug(String.format("read failed (%d/%d): %d",
						busNumber, deviceNumber, rv & 0xFF));
	...
	public void write(byte[] buffer) {
		int rv = LibUSB.libUSB.usb_bulk_write(usbDevHandle, endPointBulkOut, buffer, buffer.length, 5000); // timeout: 5 secs
		if (rv < 0) {
			LOG.debug(String.format("write failed (%d/%d): %d",
						busNumber, deviceNumber, rv & 0xFF));
	...
	public byte[] receive() {
		...
		do {
			answer= this.read();
			if ((answer[7] & (byte)0x40) == (byte)0x40) { // 01 0000 00 bmCommandStatus 1: Error
				LOG.debug(String.format("receive failed (%d/%d): %x",
						busNumber, deviceNumber, answer[8]));
				throw new RuntimeException(String.format("receive failed (%d/%d): %x",
						busNumber, deviceNumber, answer[8]));
			}
			if ((answer[7] & (byte)0x80) == (byte)0x80); { // 10 0000 00 bmCommandStatus 1: Time extension requested
				LOG.debug(String.format("receive: time extension requested (%d/%d)",
						busNumber, deviceNumber));
			}
		} while ((answer[7] & (byte)0x80) == (byte)0x80);
		LOG.debug(String.format("receive. Answer: %s", bytesToString(answer)));
		int datalength = (answer[1] & 0xFF) + ((answer[2] & 0xFF)<< 8)  + ((answer[3]&0xFF) << 16) + ((answer[4]&0xFF) << 24);
